...
def create_payment_link(telegram_id: int, tier: str) -> str:
    ...
    try:
        ...
        session = stripe.checkout.Session.create(
            ...
            mode="subscription",
            success_url=f"https://t.me/TELEGRAM_BOT_TOKEN?start=payment_success_{telegram_id} {telegram_id}",
            cancel_url=f"https://t.me/TELEGRAM_BOT_TOKEN?start=payment_cancel_{telegram_id} {telegram_id}",
        ...
        logger.error("Error creating payment link for %s: %s", telegram_id, e)
        raise
...
def create_trial_subscription(telegram_id: int, tier: str, trial_days: int = 7):
    ...
    logger.info("Trial subscription created for %s (%s, %s days)", telegram_id, tier, trial_days)
...
def handle_subscription_created(subscription_id: str, stripe_customer_id: str):
    ...
    try:
        ...
        if not telegram_id:
            logger.error("No telegram_id found for customer %s", stripe_customer_id)
            return
        ...
        if not tier:
            logger.error("Could not determine tier for subscription %s", subscription_id)
            return
        ...
        add_credits(telegram_id, credits_bonus.get(tier, 0), kind="subscription_" + tier)
        
        logger.info("Subscription %s activated for user %s (%s)", subscription_id, telegram_id, tier)
        
    except Exception as e:
        logger.error("Error handling subscription creation: %s", e)
    ...
    try:
        ...
        if subscription.status == "canceled":
            ...
            if telegram_id:
                ...
                logger.info("Subscription %s canceled for user %s", subscription_id, telegram_id)
            return
        ...
        logger.error("Error handling subscription update: %s", e)
...
def handle_invoice_payment_succeeded(invoice_id: str):
    ...
    try:
        ...
        logger.error("Error handling invoice payment: %s", e)
